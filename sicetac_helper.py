...
class SICETACHelper:
    ...
    # -------------------------------------------------------------------------
    # VERIFICAR SI UNA RUTA EXISTE (EXACTA) EN LA MATRIZ
    # -------------------------------------------------------------------------
    def ruta_existe(self, origen_input: str, destino_input: str, df_rutas: pd.DataFrame) -> bool:
        """
        Devuelve True si existe la ruta origen-destino (en cualquier sentido)
        en el DataFrame de rutas; False en caso contrario.
        """
    ...
    # -------------------------------------------------------------------------
    # SOPORTE DE LAT/LONG Y BÚSQUEDA POR DISTANCIA
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # BÚSQUEDA DE RUTA CON APROXIMACIÓN (USANDO LAT/LONG)
    # -------------------------------------------------------------------------
    def buscar_ruta_con_aproximacion(self, origen_input, destino_input, df_rutas):
        ...
        if not ruta_directa.empty:
            ...
            muni_origen_usado = self.obtener_municipio_por_codigo(
                fila["codigo_dane_origen"]
            )
            muni_destino_usado = self.obtener_municipio_por_codigo(
                fila["codigo_dane_destino"]
            )
            if muni_origen_usado is not None:
                info_aprox["nombre_origen_usado"] = (
                    f"{muni_origen_usado.get('nombre_oficial', '')} "
                    f"({muni_origen_usado.get('departamento', '')})"
                )
            if muni_destino_usado is not None:
                info_aprox["nombre_destino_usado"] = (
                    f"{muni_destino_usado.get('nombre_oficial', '')} "
                    f"({muni_destino_usado.get('departamento', '')})"
                )
        ...
        if cod_origen in codigos_origen_sice:
            ...
            cod_origen_usado = self._municipio_mas_cercano(
                cod_origen, codigos_origen_sice
            )
        ...
        if cod_destino in codigos_destino_sice:
            ...
            cod_destino_usado = self._municipio_mas_cercano(
                cod_destino, codigos_destino_sice
            )

        if cod_origen_usado is None or cod_destino_usado is None:
            info_aprox[
                "motivo"
            ] = "No fue posible encontrar municipios cercanos con rutas registradas en SICETAC."
            return None, info_aprox
        ...
        if ruta_aprox.empty:
            info_aprox[
                "motivo"
            ] = "No existe tampoco ruta entre los municipios aproximados."
            return None, info_aprox
        ...
        info_aprox[
            ...
            "Ruta exacta no encontrada. Se utilizó como referencia la ruta entre los "
            "municipios más cercanos con registro en SICETAC."
        )
